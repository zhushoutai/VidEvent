...
def weight(x, lambda_value):
    return torch.exp(-lambda_value * (x - 1)**2)

# 重新定义权重计算函数，考虑t时刻是否位于某个ground truth段内
def compute_weights_for_level_inside_segment(T, rf, gt_segments, T_total, device):
    # 初始化权重向量
    weights = torch.zeros(T,device=device)
    
    # 计算该级别每个时刻t在原视频中的实际时间点
    time_points = torch.linspace(0, T_total, steps=T,device=device)
    
    # 对于每个时刻，检查它是否位于任何一个ground truth段内
    for i, t in enumerate(time_points):
        # 检查t是否位于任何ground truth段内      
        inside_any_segment = False
        for gt_segment in gt_segments:
            if gt_segment[0] <= t <= gt_segment[1]:
                # t位于该ground truth段内
                inside_any_segment = True
                gt_length = gt_segment[1] - gt_segment[0]
                break
        
        if inside_any_segment:
            # 计算比值和权重
            ratio = min(rf,gt_length.float()) / max(rf,gt_length.float())
            weights[i] = ratio
        else:
            # t不在任何ground truth段内，可以定义一个默认行为
            # 这里我们暂时将权重设置为一个较小的值，比如使用高斯函数的最小值
            weights[i] = 0
...
class CustomTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers, norm=None):
        ...
        self.norm = norm if norm is not None else nn.LayerNorm(encoder_layer.self_attn.embed_dim)
