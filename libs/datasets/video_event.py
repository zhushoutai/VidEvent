...
#         dict_db = tuple()
#         for vid in json_db:
#             if vid["subset"].lower() not in self.split:
#                 continue
#             filename = os.path.join(self.feat_folder,vid["videoID"]+".mp4",
#                                     self.file_prefix + vid["videoID"] + ".mp4" + self.file_ext)
...
#             if "clip" in vid and len(vid["clip"]) > 0:
#                 num_acts = len(vid["clip"])
#                 segments = np.zeros([num_acts,2],dtype=np.float32)
#                 labels = np.zeros([num_acts,],dtype=np.int64)
#                 for idx,act in enumerate(vid["clip"]):
#                     segments[idx][0] = act["timeStart"]/1000 #从毫秒换算成秒
#                     segments[idx][1] = act["timeEnd"]/1000
#                     event = str(act["event"]).strip()
#                     labels[idx] = label_dict[event]
            
#             else:
#                 segments = None
#                 labels = None
            
#             dict_db += ({'id':vid["videoID"],
#                          'fps':fps,
#                          'duration':duration,
#                          'segments':segments,
#                          'labels':labels},)
...
#         # load features
#         filename = os.path.join(self.feat_folder,video_item['id']+".mp4",
#                                 self.file_prefix + video_item['id'] + ".mp4" + self.file_ext)
#         if not os.path.exists(filename):
#             print('ERROR: feature file %s not found!' % filename)
#         # with np.load(filename) as data:
#         feats = np.load(filename, allow_pickle=True).astype(np.float32)
